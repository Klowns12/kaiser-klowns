...
const HOUSE_NAMES = ["Anthovai", "LOKOVOX MEDIA", "MAVENTINE", "VELVESSENCE STUDIOS", "KURENTENGU", "Anthovai", "Lokovox Media", "Maventine", "Velvessence Studios", "KurenTengu"];
...
export default function HamburgerMenu({ scrolled = false, theme = "default" }: { scrolled?: boolean; theme?: "dark" | "default" }) {
    ...
    const primaryLinks = useMemo(() => [
        {
            name: dict.nav.group, href: '/group',
            subItems: [
                { name: dict.nav.history, href: '/group/history' },
                { name: dict.nav.mission, href: '/group/mission' },
                { name: dict.nav.governance, href: '/group/governance' },
                { name: dict.nav.keyFigures, href: '/group/key-figures' },
                { name: dict.pages.craftsmanship.title, href: '/group/craftsmanship' },
            ],
        },
        {
            name: dict.nav.houses, href: '/houses',
            subItems: [
                { name: dict.nav.allHouses, href: '/houses' },
                { name: 'Anthovai', href: '/houses/Anthovai' },
                { name: 'LOKOVOX MEDIA', href: '/houses/lokovox' },
                { name: 'MAVENTINE', href: '/houses/maventine', locked: true },
                { name: 'VELVESSENCE STUDIOS', href: '/houses/velvessence', locked: true },
                { name: 'KURENTENGU', href: '/houses/kurentengu', locked: true },
            ],
        },
        {
            name: dict.nav.commitments, href: '/commitments',
            subItems: [
                { name: dict.nav.sustainability, href: '/commitments/sustainability' },
                { name: dict.nav.socialResponsibility, href: '/commitments/social' },
                { name: dict.nav.environment, href: '/commitments/environment' },
            ],
        },
        {
            name: dict.nav.careers, href: '/careers',
            subItems: [
                { name: dict.nav.joinUs, href: '/careers' },
                { name: dict.nav.openPositions, href: '/careers#positions' },
            ],
        },
    ], [dict]);

    const secondaryLinks = useMemo(() => [
        // { name: dict.nav.investors, href: '/investors' },
        { name: dict.nav.press, href: '/press' },
        { name: dict.nav.partners, href: '/partners' },
    ...
    const handlePrimaryClick = useCallback((link: typeof primaryLinks[0]) => {
        if (link.subItems && link.subItems.length > 0) {
            setActiveSection(prev => prev === link.name ? null : link.name);
        } else {
            handleClose();
        }
    }, [handleClose]);
    ...
    const activeLink = primaryLinks.find(l => l.name === activeSection);
                    ...
                    <div className="flex flex-col h-full overflow-y-auto shrink-0" style={{ width: 'min(340px, 88vw)' }}>
                                    ...
                                    <li key={link.href}>
                                        <button
                                            ...
                                            className={classNames(
                                                ...
                                                activeSection === link.name ? 'text-foreground' : 'text-foreground hover:opacity-60',
                                                activeSection && activeSection !== link.name ? 'opacity-35' : 'opacity-100',
                                            ...
                                            <span className="flex items-center gap-3">
                                                ...
                                                {link.subItems.length > 0 && (
                                                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"
                                                        className={classNames('transition-transform duration-300 opacity-30', activeSection === link.name ? 'rotate-90 opacity-100' : '')}
                                                    >
                        ...
                        <div className="px-8 pt-2 pb-8 mt-auto">
                            <div className="flex items-center gap-5">
                                {[
                                    { name: 'Instagram', href: 'https://www.instagram.com/kaiserklowns/', d: 'M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z' },
                                    { name: 'LinkedIn', href: 'https://www.linkedin.com/company/kaiser-klowns/?viewAsMember=true', d: 'M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 01-2.063-2.065 2.064 2.064 0 112.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z' },
                                    { name: 'X', href: 'https://x.com/KaiserKlowns', d: 'M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z' },
                                ].map((social) => (
                                    <a key={social.name} href={social.href} className="text-foreground/40 hover:text-foreground/80 transition-colors duration-300" aria-label={social.name}>
                    ...
                    >
                        {activeLink && activeLink.subItems.length > 0 && (
                            <div className="px-10 pt-24 pb-10">
                                <h2 className="text-[20px] font-bold tracking-[0.08em] uppercase text-foreground mb-2">{activeLink.name}</h2>
                            ...
                            <div className="mt-5 max-h-[50vh] overflow-y-auto">
                                {searchQuery.trim() ? (() => {
                                    ...
                                    const allPages = [
                                        ...
                                        { name: 'Anthovai', href: '/houses/Anthovai' },
                                        { name: 'Lokovox Media', href: '/houses/lokovox' },
                                        { name: 'Maventine', href: '/houses/maventine' },
                                        { name: 'Velvessence Studios', href: '/houses/velvessence' },
                                        { name: 'KurenTengu', href: '/houses/kurentengu' },
                                        { name: dict.nav.commitments, href: '/commitments' },
                                        { name: dict.nav.sustainability, href: '/commitments/sustainability' },
                                        { name: dict.nav.socialResponsibility, href: '/commitments/social' },
                                        { name: dict.nav.environment, href: '/commitments/environment' },
                                        ...
                                        { name: dict.nav.partners, href: '/partners' },
                                        { name: dict.nav.contact, href: '/contact' },
                                        { name: dict.news.title, href: '/news' },
                                        { name: dict.pages.formulaK.title, href: '/news/formula-k-partnership' },
                                        { name: dict.pages.maventineTokyo.title, href: '/news/maventine-tokyo' },
                                        { name: dict.pages.AnthovaiPlatform.title, href: '/news/Anthovai-platform' },
                                        { name: dict.pages.arkaiNews.title, href: '/news/arkai-work-assistant' },
                                        { name: dict.pages.arkaiPromo.title, href: '/news/arkai-early-access' },
                                    ];
                                    const results = allPages.filter(p => p.name.toLowerCase().includes(q));
