...
export default function HouseDetailClient({ slug, houseConfig }: HouseDetailClientProps) {
    ...
    const displayedProjects = house.projects 
        ? (showAllProjects ? house.projects : house.projects.slice(0, 12)) 
        : [];
            ...
            <section className="py-16 md:py-24 bg-background">
                <div className="max-w-7xl mx-auto px-6 md:px-12 grid grid-cols-1 lg:grid-cols-3 gap-12">
                    ...
                    <div className="space-y-6">
                        {[
                            { name: houseDetails.foundedLabel, value: house.founded },
                            { name: houseDetails.headquartersLabel, value: house.headquarters },
                            { name: houseDetails.employeesLabel, value: house.employees },
                            { name: houseDetails.sectorLabel, value: house.sector },
                        ].map((item) => (
                            <div key={item.name} className="border-l-2 pl-5" style={{ borderColor: houseConfig.accent + '40' }}>
                                <p className="text-[10px] tracking-[0.2em] uppercase text-foreground/35">{item.name}</p>
                        ...
                        {house.projects.length > 12 && (
                            <div className="mt-12 flex justify-center">
                                <button
                                    ...
                                    className="px-6 py-3 text-[11px] tracking-[0.15em] uppercase border border-foreground/20 text-foreground/70 hover:border-foreground hover:text-foreground transition-all duration-300 font-bold"
                                >
                                    {showAllProjects ? houseDetails.showLessProjects : houseDetails.viewMoreProjects}
                                </button>
                            </div>
                        )}
                    </div>
